...
describe("Collection endpoint prepare to tests", () => {
  ...
  it("should update collection", async () => {
    ...
    const updated: ResCollectionDto = await adapter.updateCollection(
  ...
  it("should deleted current collection", async () => {
    ...
    const lastCollection: ResCollectionDto =
      collections.collections[collections.collections.length - 1];
    await adapter.deleteCollection(lastCollection.id!);

    const deletedCollection: ResCollectionDto = await adapter.getCollection(
      lastCollection.id,
    );
