...
export class CollectionMockAdapter extends CollectionRepository {
  ...
  async createCollection(data: ICollection): Promise<ResCollectionDto> {
    logger.verbose(
      `CollectionMockAdapter was called createCollection method with param - ${JSON.stringify(data)}`,
    );

    this.collections.push(data);

    logger.verbose(
      `collections - ${JSON.stringify(this.collections)}, ${JSON.stringify(data)}`,
    );
  ...
  async getCollection(collectionId: string): Promise<ResCollectionDto | null> {
    logger.verbose(
      `CollectionMockAdapter was called getCollection method with param - ${JSON.stringify(collectionId)}`,
    );
  ...
  async getCollections(): Promise<ResCollectionsDto> {
    logger.verbose(
      "CollectionMockAdapter was called getCollections without any params ",
    );
  ...
  async deleteCollection(collectionId: string): Promise<void> {
    logger.verbose(
      `CollectionMockAdapter was called deleteCollection method with param - ${JSON.stringify(collectionId)}`,
    );
