...
describe("FavoriteAdapter", () => {
  ...
  describe("Favorite Adapter", () => {
    ...
    it("should create favorite-relation", async () => {
      ...
      expect(
        await setupUserProductRelations().then((data) => data.id),
      ).toBeDefined();
      expect(
        typeof (await setupUserProductRelations().then((data) => data.id)),
      ).toBe("string");
    });
    ...
    it("should get current relation by identifier", async () => {
      ...
      try {
        ...
        const user: ResCreatedUserDto =
          await userService.createUser(MockDataReqUser);

        try {
          product = await productService.createProduct(MockDataReqProduct);
        } catch (e) {
          const collection: ResCollectionDto =
            await collectionService.createCollection(MockDataReqCollection);
    ...
    it("should update relation", async () => {
      ...
      const createdUserId = await userService
        .getUserByLogin(createdUser.login)
        .then((res) => res.id);
      const updated: ResRelationDto = await adapter.updateRelation(
    ...
    it("should delete relation", async () => {
      ...
      await adapter.deleteRelation(
        relations.relations[relations.relations.length - 1].id!,
      );

      const deletedRelation: ResRelationDto = await adapter.getRelation(
        relations.relations[relations.relations.length - 1].id!,
      );
  ...
  async function setupUserProductRelations(): Promise<ResRelationDto> {
    ...
    if (!products.products?.length) {
      const collections: ResCollectionsDto =
        await collectionService.getCollections();

      if (!collections.collections?.length) {
        const createdCollection: ResCollectionDto =
          await collectionService.createCollection({ name: "Test collection" });
