...
export class BucketAdapter extends BucketRepository {
  ...
  async createBucket(data: ReqCreateBucketDto): Promise<ResBucketDto> {
    logger.verbose(
      `BucketAdapter called createBucket method with param - ${JSON.stringify(data)}`,
    );
  ...
  async getBuckets(
    ...
    logger.verbose(
      `BucketAdapter called getBuckets method with params - ${JSON.stringify(userId)}, ${JSON.stringify(productId)}`,
    );
    return {
  ...
  async updateBucket(
    ...
    logger.verbose(
      `BucketAdapter called updateBucket method with param - ${JSON.stringify(id)}, ${JSON.stringify(data)}`,
    );
  ...
  async clearBucket(id: string): Promise<void> {
    logger.verbose(
      `BucketAdapter called clearBucket method with param - ${JSON.stringify(id)}`,
    );
    throw new Error();
