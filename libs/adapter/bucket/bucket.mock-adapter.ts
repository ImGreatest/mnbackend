...
export class BucketMockAdapter extends BucketRepository {
  ...
  async createBucket(data: ReqCreateBucketDto): Promise<ResBucketDto> {
    logger.verbose(
      `BucketMockAdapter was called createBucket with param - ${JSON.stringify(data)}`,
    );
  ...
  async getBuckets(
    ...
    logger.verbose(
      `BucketMockAdapter was called getBuckets with params - ${JSON.stringify(userId)}, ${JSON.stringify(productId)}`,
    );
  ...
  async updateBucket(
    ...
    logger.verbose(
      `BucketMockAdapter was called updateBucket with params - ${JSON.stringify(id)}, ${JSON.stringify(data)}`,
    );
  ...
  async clearBucket(id: string): Promise<void> {
    logger.verbose(
      `BucketMockAdapter was called with params - ${JSON.stringify(id)}`,
    );
