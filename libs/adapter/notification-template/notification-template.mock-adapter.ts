...
import { NotificationTemplateRepository } from "../../domain/notification/repository/notification-template.repository";
...
export class NotificationTemplateMockAdapter extends NotificationTemplateRepository {
	...
	async createTemplate(data: ReqCreateNotificationTemplateDto): Promise<ResNotificationTemplateDto> {
		throw new Error(`NotificationTemplateMockAdapter called createTemplate with param - ${JSON.stringify(data)}`);
	}

	async getTemplate(id: string): Promise<ResNotificationTemplateDto> {
		throw new Error(`NotificationTemplateMockAdapter called getTemplate method with param - ${JSON.stringify(id)}`);
	}

	async getTemplates(type?: ENotificationType): Promise<ResNotificationTemplatesDto> {
		if (type) {
			throw new Error(`NotificationTemplateMockAdapter called getTemplates method with param - ${JSON.stringify(type)}`);
		} else {
			throw new Error('NotificationTemplateMockAdapter called getTemplates without params');
		}
	}

	async updateTemplate(id: string, data: ReqUpdateNotificationTemplateDto): Promise<ResUpdateNotificationTemplateDto> {
		throw new Error(`NotificationTemplateMockAdapter called updateTemplate with params - ${JSON.stringify(id)}, ${JSON.stringify(data)}`);
	}

	async deleteTemplate(id: string): Promise<void> {
		throw new Error(`NotificationTemplateMockAdapter called deleteTemplate method with param - ${JSON.stringify(id)}`);
	}
}