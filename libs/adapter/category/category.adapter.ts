...
export class CategoryAdapter extends CategoryRepository {
  ...
  async createCategory(data: ReqCreateCategoryDto): Promise<ResCategoryDto> {
    logger.verbose(
      `CategoryAdapter called createCategory method with param - ${JSON.stringify(data)}`,
    );
  ...
  async getCategory(id: string, name?: string): Promise<ResCategoryDto> {
    logger.verbose(
      `CategoryAdapter called getCategory method with param - ${JSON.stringify(id)}, ${JSON.stringify(name)}`,
    );
  ...
  async getCategories(): Promise<ResCategoriesDto> {
    logger.verbose(
      "CategoryAdapter called getCategories method without any params",
    );
  ...
  async deleteCategory(id: string): Promise<void> {
    logger.verbose(
      `CategoryAdapter called deleteCategory with param - ${JSON.stringify(id)}`,
    );
