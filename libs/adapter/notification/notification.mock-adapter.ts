...
export class NotificationMockAdapter extends NotificationRepository {
  ...
  async createNotification(
    ...
    throw new Error(
      `NotificationMockAdapter called createNotification with param - ${JSON.stringify(data)}`,
    );
  }

  async getNotification(id: string): Promise<ResNotificationDto> {
    throw new Error(
      `NotificationMockAdapter called getNotification with param - ${JSON.stringify(id)}`,
    );
  }
  ...
  async getNotifications(
    ...
    throw new Error(
      `NotificationMockAdapter called getNotifications with params - ${JSON.stringify(userId)}, ${JSON.stringify(templateId)}`,
    );
  }
}
